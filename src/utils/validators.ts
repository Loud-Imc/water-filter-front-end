...
export const customerSchema = yup.object({
  ...
  contact: yup.string().min(10, 'Contact must be at least 10 digits').required('Contact is required'),
  regionId: yup.string().required('Region is required'),
});

export const roleSchema = yup.object({
  name: yup.string().min(2, 'Role name must be at least 2 characters').required('Role name is required'),
  permissions: yup.object(),
});

export const regionSchema = yup.object({
  name: yup.string().min(2, 'Region name must be at least 2 characters').required('Region name is required'),
});
